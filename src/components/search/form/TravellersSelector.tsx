...
export function TravellersSelector({ initialPax = "1", initialClass = "ECONOMY" }: TravellersSelectorProps) {
    ...
    const updateCount = (type: PassengerType, delta: number) => {
        setCounts(prev => {
            ...
            // Ensure at least 1 adult/student/youth if total is becoming 0? 
            // Actually usually at least 1 traveler total is required.
    ...
    // Ensure total pax is at least 1
    useEffect(() => {
        if (totalPax === 0) {
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="absolute top-[calc(100%+8px)] right-0 w-[320px] bg-[#1a1a24] border border-white/10 rounded-2xl shadow-2xl p-4 z-50 overflow-hidden"
                    >
